...
class PersistedState {
    ...
    initIpcConnectionToMain() {
        electron_1.ipcRenderer.send(helpers_1.ipcEvents.CONNECT);
        electron_1.ipcRenderer.on(helpers_1.ipcEvents.COMMIT, (_event, { type, payload, options }) => {
            this.store.commit(type, payload, options);
        });
        electron_1.ipcRenderer.on(helpers_1.ipcEvents.DISPATCH, (_event, { type, payload, options }) => {
            this.store.dispatch(type, payload, options);
        });
        electron_1.ipcRenderer.on(helpers_1.ipcEvents.CLEAR_STATE, () => {
            this.clearState();
        });
        // ipcRenderer.answerMain(ipcEvents.GET_STATE, () => {
        // 	return this.store.state
        // })
        electron_1.ipcRenderer.on(helpers_1.ipcEvents.GET_STATE, (event) => {
            event.sender.send(helpers_1.ipcEvents.GET_STATE, this.store.state);
            // return this.store.state
        });
    }
    /**
    ...
    static getStoreFromRenderer() {
        ...
        const storePromise = new Promise((resolve) => {
            ...
            const getState = () => {
                if (!connection)
                    throw new Error('[Vuex Electron] Not connected to renderer.');
                const win = electron_2.BrowserWindow.fromWebContents(connection);
                if (!win)
                    throw new Error('[Vuex Electron] Cannot get BrowserWindow from WebContents.');
                const pr = new Promise((res) => {
                    electron_1.ipcMain.on(helpers_1.ipcEvents.GET_STATE, (event, data) => {
                        res(data);
                    });
                });
                win.webContents.send(helpers_1.ipcEvents.GET_STATE);
                return pr;
            };
            const clearState = () => {
                if (!connection)
            ...
            electron_1.ipcMain.on(helpers_1.ipcEvents.CONNECT, (event) => {
